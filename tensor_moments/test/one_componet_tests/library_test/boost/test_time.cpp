...
int main(int argc, char const *argv[]) {
	size_t n_rec = atoul(argv[1]);
	size_t sources_count = atoul(argv[2]);
	size_t n_samples = atoul(argv[3]);
	...
	compute(rec_samples_data3D, rec_coords_data2D, sources_coords_data2D, sources_times_data2D, tensor_matrix_data1D, data);
