    ...
    #pragma omp parallel
    {
        ...
        for (size_t r_ind = 0; r_ind < n_rec-(n_rec%vector_dim); r_ind+=vector_dim) {
            for (size_t i = 0; i < 3; ++i) {
                vect_rec_coord[(r_ind/vector_dim)*3+i] = _mm256_set_ps(
        ...
        for (size_t i = 0; i < sources_count; ++i) {
            for (size_t r_ind = 0; r_ind < n_rec-(n_rec%vector_dim); r_ind+=vector_dim) {
                for (size_t crd = 0; crd < 3; ++crd) {
                    coord_vec[crd] = _mm256_sub_ps(vect_rec_coord[(r_ind/vector_dim)*3+crd], _mm256_set1_ps(sources_coords_(Array2D_ind{{i, crd}})));
                }
...
void AmplitudesCalculatorM256<double>::realize_calculate() {
    ...
    #pragma omp parallel
    {
        ...
        for (size_t r_ind = 0; r_ind < n_rec-(n_rec%vector_dim); r_ind+=vector_dim) {
            for (size_t i = 0; i < 3; ++i) {
                vect_rec_coord[(r_ind/vector_dim)*3+i] = _mm256_set_pd(
                                                                       rec_coords_(Array2D_ind{{r_ind+3, i}}), rec_coords_(Array2D_ind{{r_ind+2, i}}), 
                                                                       rec_coords_(Array2D_ind{{r_ind+1, i}}), rec_coords_(Array2D_ind{{r_ind, i}})
                                                                      );
            }
        ...
        for (size_t i = 0; i < sources_count; ++i) {
            for (size_t r_ind = 0; r_ind < n_rec-(n_rec%vector_dim); r_ind+=vector_dim) {
                for (size_t crd = 0; crd < 3; ++crd) {
                    coord_vec[crd] = _mm256_sub_pd(vect_rec_coord[(r_ind/vector_dim)*3+crd], _mm256_set1_pd(sources_coords_(Array2D_ind{{i, crd}})));
                }
