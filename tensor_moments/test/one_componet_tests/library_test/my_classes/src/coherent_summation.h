...
void compute(const Array2D<T> &data, const Array2D<T> &rec_coords, const Array2D<T> &sources_coords, 
             const Array2D<R> &sources_times, const T *tensor_matrix, T *result_data) {
	ptrdiff_t rec_block_size = 60;
    ptrdiff_t samples_block_size = 400;
    ptrdiff_t n_samples = data.get_x_dim();
    ptrdiff_t n_rec = data.get_y_dim();
    ptrdiff_t sources_count = sources_coords.get_y_dim();

    T *amplitudes_buf = new T[sources_count*n_rec]();
    Array2D<T> amplitudes{amplitudes_buf, sources_count, n_rec};
