    ...
    #pragma omp parallel
    {
        ...
        for (ptrdiff_t r_ind = 0; r_ind < n_rec-(n_rec%vector_dim); r_ind+=vector_dim) {
            for (ptrdiff_t i = 0; i < 3; ++i) {
                vect_rec_coord[(r_ind/vector_dim)*3+i] = _mm256_set_ps(rec_coords_(r_ind+7, i), rec_coords_(r_ind+6, i), rec_coords_(r_ind+5, i), rec_coords_(r_ind+4, i),
        ...
        for (ptrdiff_t i = 0; i < sources_count; ++i) {
            for (ptrdiff_t r_ind = 0; r_ind < n_rec-(n_rec%vector_dim); r_ind+=vector_dim) {
                for (ptrdiff_t crd = 0; crd < 3; ++crd) {
                    coord_vec[crd] = _mm256_sub_ps(vect_rec_coord[(r_ind/vector_dim)*3+crd], _mm256_set1_ps(sources_coords_(i, crd)));
                }
...
void AmplitudesCalculatorM256<double>::realize_calculate() {
    ...
    #pragma omp parallel
    {
        ...
        for (ptrdiff_t r_ind = 0; r_ind < n_rec-(n_rec%vector_dim); r_ind+=vector_dim) {
            for (ptrdiff_t crd = 0; crd < 3; ++crd) {
                vect_rec_coord[(r_ind/vector_dim)*3+crd] = _mm256_set_pd(rec_coords_(r_ind+3, crd), rec_coords_(r_ind+2, crd), rec_coords_(r_ind+1, crd), rec_coords_(r_ind+0, crd));
            }
        }
