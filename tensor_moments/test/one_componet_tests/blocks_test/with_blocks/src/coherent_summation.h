            ...
            for (ptrdiff_t bl_r = 0; bl_r < n_rec; bl_r += rec_block_size) {
                for (ptrdiff_t i = 0; i < sources_count; ++i) {
                    for (ptrdiff_t r_ind = bl_r; r_ind < std::min(bl_r+rec_block_size, n_rec); ++r_ind) {
                        ...
                        result_data.row(i).template segment(bl_t, std::min(bl_t+samples_block_size, n_samples-ind)-bl_t) += ampl_tmp*data.row(r_ind).template segment(bl_t+ind, std::min(bl_t+samples_block_size, n_samples-ind)-bl_t);
                    }
                }
            }
        }
